...
app.get('/api/pins', async (req, res) => {
    const userId = req.query.userId || "USER-001"; // USUARIO DE TEST; SE LLAMA ALEX P.
...
app.post('/api/pins/:id/like', async (req, res) => {
    ...
    try {
        const checkResult = await session.run(
            `MATCH (u:User {id_user: $userId})-[r:LIKES]->(p:Pin {id_pin: $pinId}) RETURN r`,
            { userId, pinId }
        );
        ...
        if (checkResult.records.length > 0) {
            await session.run(
                `MATCH (u:User {id_user: $userId})-[r:LIKES]->(p:Pin {id_pin: $pinId}) DELETE r`,
                { userId, pinId }
            );
            isLiked = false;
        } else {
